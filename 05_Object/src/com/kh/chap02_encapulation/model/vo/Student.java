...
public class Student {
	...
	// 이때, 이 메소드는 접근 가능하도록 해야하기 때문에 public 접근 제한자를 사용
	
	// 이름값을 기록 및 수정할 수 있는 기능의 메소드(name필드에 값을 넣기위한 용도)
	// set+필드명=> 두 단어를 이어줄대 카멀케이스(낙타등표기법)활용하여 메서드명 선언
	...
	public void setName(String name) {
		...
		//왜? 필드명과 매개변수명이 동일한 경우 {}영역 내에서는 해당 영역에서 만들어진 변수가 좀더 우선순위가 높기때문에
	...
	// 키값을 기록 및 수정할 수 있는 기능의 메소드(height 필드에 값을 대입하기위한 용도)
	...
	//데이터를 반환 해주는 기능의 메소드 : getter 메소드
	//name 필드에 담긴 값을 돌려주는 용도의 메소드
	//get+필드명(카멀케이스 적용)
	
	public String getName() { //void : 반환하는 값이 없을 경우에 작성
		
		return name; // return + 반환값; -> 반환값을 나를 호출한 메서드의 위치에 전달하겠다는 뜻.
