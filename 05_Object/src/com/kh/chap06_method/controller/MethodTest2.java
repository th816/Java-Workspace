...
public class MethodTest2 {
	...
	public static String method2() {
		...
		return "매개변수는 없구요, 반환값은 있는 메소드입니다";
	}
	...
	public static int method4(int num1) {
		 ...
		 * 스태틱 메소드에서는 프로그램 시작시 정적 메모리영역에 들어가게 되는데
		 * 이때 스태틱으로 선언된 필드(전역변수)도 함게 정적메모리 영역에 들어갈것
		 * 하지만 스태틱으로 선언되지 않은 전역변수는 스태틱 메모리에 올라가는 시점에
		 * 사용이 불가능하므로 에러가 발생함.
		 * 
		 */
		
		
		//static 메소드에서는 static필드만 사용이 가능하다
