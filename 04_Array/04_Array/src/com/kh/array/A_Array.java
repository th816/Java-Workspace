...
//			ex) int a = 10;
//				a = 20;
//	배열 : 하나의 공간에 여러개의 값을 담을수 있음(단, 같은 자료형의 값으로만 가능
//		정확히 얘기하자면 배열의 각 인데스 자리에 실제값이 담김(인덱스  0 부터 시작.)
//		ex) int[] a = new int[3]; // int형의 자료가 들어갈수 있는 공간을 3개 만든다.
//								// 방, 사물함 [0] [1] [2]을 3개 만듬.
//								// 인데스는 하상 0부터 시작한다.(중요)
//								// 배열의 크기는 3, 마지막 인덱스 = 2
//								// 배열의 크기 100, 마지막 인데스 = 99
//								// 항상 크기 = n, 마지막 인덱스 = n-1
...
//		5칸짜리 방이 만들어진후 각방에 기본값으로(int 0으로) 데이터가 초기화되고, 주소값이 할당됨(0x123)
...
//		=> 실제 값(리터럴)을 바로 stack영역에 담을수 있는 변수 => 일반변수
//		
...
//		내가 각 인덱스 초기화하지 않았는데도 값들이 감겨있는 이유
//		-> heap 라는 영역에는 절~때로 빈공간이 존재할수 없기때문.
...
//		각 배열의 인덱스에 값을 대입하는 반복문
		for (int i = 0; i < arr.length; i++) {
			arr[i] = i + 1;// arr[0] = 1, arr[1] = 2, arr[2] = 3, arr[3] = 4, arr[4]
		}
...
//		int i2 = null; typeMissMatch -> null값을 int 자료형에 넣을수 없음.
...
//		=> 배열의 크기를 변겨앟고자 한다면 어쩔수없이 새로운 배열을 다시 만들어야함.

		System.out.println("====== arr 변경후 =======");
		System.out.println(arr);
		System.out.println("arr 의 해시코드 값 : " + arr.hashCode());
...
//		주소값 == 주소값 : 주소값간에 동등비교를 수행하기 때문에 false
	}
